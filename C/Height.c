...
int main(int argc, char** argv)
{
	...
	while (input[0] != 'q')
	{
		...
		avg = averageIntegerArray(heights, n);
		min = findExtremeInIntegerArray(MIN, heights, n);
		max = findExtremeInIntegerArray(MAX, heights, n);
		printf("%.2f %d %d\n", avg, min, max);
		free(heights);
		heights = NULL;
...
int findExtremeInIntegerArray(int minOrMax, int values[], int count)
{
	...
	{
		if ((minOrMax == MIN && values[i] < extreme ) || (minOrMax == MAX && values[i] > extreme ))
		{
			extreme = values[i];
		}
