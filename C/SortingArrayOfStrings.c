...
int utilityFunctionToCalcDistinctCharacters(const char* str){
    int cnt=0;
    if(str == NULL) return -1;
    for(int i=0; i<strlen(str); i++){
        int charAppeared = 0; // false_Condition
        for(int j=0; j<i; j++){
            if(str[j] == str[i]) { 
                charAppeared=1; 
                break; 
...
int sort_by_number_of_distinct_characters(const char* a, const char* b) {
    int distinctCharsCount = utilityFunctionToCalcDistinctCharacters(a) - utilityFunctionToCalcDistinctCharacters(b);
    if(distinctCharsCount) { return distinctCharsCount; }
    else{
